...
#
# tf 文档构建配置文件，由 sphinx-quickstart 在 2009 年 6 月 1 日创建。
#
...
# 您正在文档化的项目的版本信息，用作替换
# |version| 和 |release|，也用于整个构建文档中的其他地方。
#
# 短 X.Y 版本。
version = '0.1'
...
# Sphinx 自动生成内容的语言。请参阅文档
# 以获取支持语言的列表。
# language = None

# 替换 |today| 有两种选择：要么将 today 设置为某个
# 非假值，然后使用它：
# today = ''
...
# 单元标题（例如 .. function::）的前面。
# add_module_names = True

# 如果为 true，将在输出中显示 sectionauthor 和 moduleauthor 指令。
# 它们默认被忽略。
# show_authors = False
...
# 图像文件的名称（相对于此目录）放置在侧边栏的顶部。
# html_logo = None

# 图像文件的名称（在静态路径内）用作文档的 favicon。
# 此文件应为 16x16 或 32x32 像素大小的 Windows 图标文件 (.ico)。
# html_favicon = None

# 在此处添加包含自定义静态文件（如样式表）的任何路径，
# 相对于此目录。它们在内置静态文件之后复制，
# 因此名为 "default.css" 的文件将覆盖内置的 "default.css" 文件。
html_static_path = ['_static']

# 如果不为空，则在每个页面底部插入“上次更新时间: ”时间戳，
# 使用给定的 strftime 格式。
...
# 对于“manual”文档，如果此为 true，则顶级标题是部分，
# 而不是章节。
