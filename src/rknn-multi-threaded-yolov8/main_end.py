...
def mid(follow, mask):
    ...
    for y in range(height - 1, -1, -1):
        if (mask[y][max(0, half - half_width):half] == np.zeros_like(
            ...
            cv2.circle(follow, (int(left), y), 1, (255, 255, 255), -1)
        else:
        ...
        if (mask[y][half:min(width, half + half_width)] == np.zeros_like(
            ...
            cv2.circle(follow, (int(right), y), 1, (255, 255, 255), -1)
        else:
        ...
        cv2.circle(follow, (mid, y), 1, (255, 255, 255), -1)  # mid_line

        if y == 150:  # 设置前瞻点位置
            mid_output = int(mid)
            cv2.circle(follow, (mid_output, 150), 5, (255, 255, 255), -1)  # opencv为(x,y),画出指定提取中点
    ...
    return error, mid_angle  # error为正数右转,为负数左转；角度同理
...
if __name__ == "__main__":
    ...
    while (cap.isOpened()): # 循环直到摄像头关闭
        ...
        if mission_flag == 1:
            ...
            # 进行高斯滤波，灰度，自适应阈值处理
            gauss = cv2.GaussianBlur(gray, (3, 3), 0) # 对灰度图进行高斯滤波
            # #地面不反光使用
            _, mask = cv2.threshold(gauss, 200, 255, cv2.THRESH_BINARY) # 对灰度图进行二值化处理，阈值为200
            ...
            lines = cv2.HoughLinesP(mask, 1, np.pi / 180, threshold=350, minLineLength=100, maxLineGap=10) # 使用霍夫变换检测直线
            if lines is not None: # 如果检测到直线
                for line in lines: # 遍历每条直线
                    ...
                    if (abs(angle) < 10 or abs(angle - 180)) < 10 and (200 <= y1 <= 220 or 200 <= y2 <= 220):  # 设置角度阈值，例如小于10度或接近180度，并检查Y坐标范围
                        stop_flag = 1 # 设置停止标志
